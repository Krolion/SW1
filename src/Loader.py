    ...
    def __init__(self):
        ...
        Args:
            ...
            transform (callable, optional): Optional transform to be applied
                on a sample.
        """
        tk = os.listdir("./MyData")
        self.data  =  [(image_loader("./MyData/" + tk[i]),torch.tensor(i))  for i in range(len(tk))]
...
train_loader = torch.utils.data.DataLoader( FU ,
    batch_size=args.batch_size, shuffle=True, **kwargs)
	
test_loader = torch.utils.data.DataLoader( FU ,
    batch_size=args.batch_size, shuffle=True, **kwargs)
        ...
        for i, (data, _) in enumerate(test_loader):
            ...
            if i == 0:
                ...
                save_image(comparison.cpu(),
                         '../bin/results/reconstruction_' + str(epoch) + '.png', nrow=n)
...
if __name__ == "__main__":
    for epoch in range(1, args.epochs + 1):
        ...
        with torch.no_grad():
            ...
            save_image(sample.view(4,1,128, 128),
                       '../bin/results/sample_' + str(epoch) + '.png')